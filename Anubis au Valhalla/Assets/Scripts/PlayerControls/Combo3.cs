...
public class Combo3 : MonoBehaviour
{
    ...
    public virtual void OnTriggerEnter2D(Collider2D col)
    {
        if (col.CompareTag("Monstre") && col.GetComponentInParent<MonsterLifeManager>().isInvincible == false)
        {
            ...
            else if(!col.gameObject.GetComponentInParent<MonsterLifeManager>().isMomified && col.gameObject.GetComponentInParent<MonsterLifeManager>().elite && !col.gameObject.GetComponentInParent<MonsterLifeManager>().isBoss)
            {
                col.gameObject.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
